...
static char *_get_default_account(uint32_t user_id)
{
	...
	user.uid = user_id;
	if (assoc_mgr_fill_in_user(acct_db_conn, &user, 0, NULL) != SLURM_ERROR) {
...
extern int job_submit(struct job_descriptor *job_desc, uint32_t submit_uid)
{
  ...
  debug("default_account: starting plugin") ;
    ...
    debug(
        "default_account: partition %s set by command line",
        job_desc->partition
        );
  ...
  if (job_desc->partition != NULL ){
    ...
    while ((part_ptr = (struct part_record *) list_next(part_iterator))) {
      if (part_ptr->flags & PART_FLAG_DEFAULT){
        debug("default_account: found default partition %s", part_ptr->name);
  ...
  if (job_desc->partition == NULL){
    debug(
        "default_account: no partition set and no match found for %s",
        account
        );
  }
  return SLURM_SUCCESS;
